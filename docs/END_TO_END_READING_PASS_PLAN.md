...
Phase A contains only changes whose intended result was already settled by the
reading-pass notes and which do not depend on merging phases or rebuilding the
research closures:

- remove or replace predetermined stale copy;
- apply the settled small card corrections;
- move the existing Space Warper card from WARP to GREEN;
- add the three-type producer legend and card-map color coding;
