...
const mode = argumentsList.includes("--write")
  ? "write"
...
if (!mode || !assetMapArgument) {
  console.error(
    "Usage: node scripts/apply-proto-icons.mjs (--write|--check) --asset-map <recognized-game-assets.json>",
  );
  process.exit(2);
}
...
if (map.schemaVersion !== 2)
  throw new Error(`Unsupported asset-map schema ${map.schemaVersion}.`);
...
const mapAdditions = [
  {
    ...
    asset: "accumulator-full.png",
    guideAliases: ["charged Accumulator", "charged Accumulators"],
  },
  { id: 1407, name: "Engine", asset: "engine.png" },
  {
    id: 2213,
    name: "Geothermal Power Station",
    asset: "geothermal-power-station.png",
  },
];
const effectiveItems = map.items
  .map((item) =>
    itemAssetCorrections.has(Number(item.id))
      ? { ...item, asset: itemAssetCorrections.get(Number(item.id)) }
      : item,
  )
  .concat(
    mapAdditions.filter(
      (addition) => !map.items.some((item) => Number(item.id) === addition.id),
    ),
  );

const itemById = new Map(effectiveItems.map((item) => [Number(item.id), item]));
const technologyById = new Map(
  map.technologies.map((technology) => [Number(technology.id), technology]),
);
if (itemById.size !== effectiveItems.length)
  throw new Error("Duplicate item ID in effective asset map.");
if (technologyById.size !== map.technologies.length)
  throw new Error("Duplicate technology ID in asset map.");

const toWebPath = (value) => value.replaceAll("\\", "/").replace(/^\.\//, "");
const itemAsset = (item) => toWebPath(`${map.assetRoots.items}/${item.asset}`);
const technologyAsset = (technology) =>
  toWebPath(`${map.assetRoots.technologies}/${technology.asset}`);
const escapeRegExp = (value) => value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
const escapeAttribute = (value) =>
  value.replaceAll("&", "&amp;").replaceAll('"', "&quot;");

for (const item of effectiveItems) {
  if (!fs.existsSync(path.join(root, itemAsset(item))))
    throw new Error(`Missing mapped item asset: ${itemAsset(item)}`);
}
for (const technology of map.technologies) {
  if (!fs.existsSync(path.join(root, technologyAsset(technology))))
    throw new Error(
      `Missing mapped technology asset: ${technologyAsset(technology)}`,
    );
}
...
for (const item of effectiveItems) {
  ...
  for (const label of labels) {
    ...
    if (existing && existing.id !== item.id)
      throw new Error(`Ambiguous item label "${label}" in asset map.`);
...
const itemPattern = [...itemLabels.keys()]
  ...
  .map(escapeRegExp)
  .join("|");
const makeItemMatcher = () =>
  new RegExp(`(?<![A-Za-z0-9])(?:${itemPattern})(?![A-Za-z0-9])`, "giu");
...
for (const facility of facilityByRecipeType.values()) {
  if (!itemById.has(facility.itemId))
    throw new Error(
      `Producer ${facility.itemId} is missing from the asset map.`,
    );
}
...
function selectRecipe(inputText, outputText) {
  ...
  if (!outputIds.length)
    throw new Error(
      `No mapped output item found after production arrow: "${outputText}".`,
    );
  ...
  for (const outputId of outputIds) {
    ...
    if (!candidates.length)
      throw new Error(
        `No authoritative recipe produces mapped item ${outputId}.`,
      );
    const ranked = candidates
      .map((recipe) => {
        ...
        const overlap = recipeInputs.filter((itemId) =>
          inputIds.has(itemId),
        ).length;
        const missing = recipeInputs.filter(
          (itemId) => !inputIds.has(itemId),
        ).length;
        return {
          recipe,
          score: overlap * 20 - missing * 3 + (missing === 0 ? 1000 : 0),
        };
      })
      .sort(
        (left, right) =>
          right.score - left.score ||
          Number(left.recipe.recipe_id) - Number(right.recipe.recipe_id),
      );
  ...
  if (types.size !== 1) {
    throw new Error(
      `Outputs after arrow require different producer types: "${outputText}".`,
    );
  }
  return selected[0];
}
...
function arrowMarkup(recipe) {
  ...
  if (!facility)
    throw new Error(
      `No baseline producer for recipe type ${recipe.recipe_type}.`,
    );
...
function materializeProductionArrows(html) {
  if (html.includes('class="production-arrow"'))
    return { html, sourceArrows: 0, outputArrows: 0, normalizedRoutes: 0 };
  let sourceArrows = 0;
  let outputArrows = 0;
  let normalizedRoutes = 0;
  const transformed = html.replace(
    /<span class="route-chain">([\s\S]*?)<\/span>/g,
    (full, originalInner) => {
      ...
      if (parts.length === 1)
        return `<span class="route-chain">${inner}</span>`;
      ...
      for (let index = 0; index < parts.length - 1; index += 1) {
        if (override?.recipeIds[index]) {
          ...
          if (!recipe)
            throw new Error(
              `Route override recipe ${override.recipeIds[index]} is missing.`,
            );
          recipesForArrows.push(recipe);
          continue;
        }
        const inputText = stripTags(parts[index]).split(/[;·]/).at(-1).trim();
        const outputText = stripTags(parts[index + 1])
          .split(/[;·]/)[0]
          .trim();
...
function phaseIcon(phase, variant) {
  ...
  const record =
    binding.kind === "item"
      ? itemById.get(binding.id)
      : technologyById.get(binding.id);
  if (!record)
    throw new Error(
      `Phase ${phase} binding ${binding.id} is missing from asset map.`,
    );
  const source =
    binding.kind === "item" ? itemAsset(record) : technologyAsset(record);
...
function addStructuralIcons(html) {
  let transformed = html.replace(
    /<a(?=[^>]*\bclass="[^"]*\brail-tab\b[^"]*")(?=[^>]*\bdata-phase="([^"]+)")[^>]*>/g,
    (opening, phase, offset, source) => {
      if (
        source
          .slice(offset + opening.length)
          .startsWith('<img class="phase-icon phase-icon-rail"')
      )
        return opening;
  ...
  transformed = transformed.replace(
    /<a(?=[^>]*\bclass="[^"]*\bphase-tag\b[^"]*")(?=[^>]*\bhref="#([^"]+)")[^>]*>/g,
    (opening, phase, offset, source) => {
      if (!phaseBindings.has(phase)) return opening;
      if (
        source
          .slice(offset + opening.length)
          .startsWith('<img class="phase-icon phase-icon-tag"')
      )
        return opening;
...
function addTechnologyIcons(html) {
  return replaceElementsByClass(
    ...
    (reference) => {
      ...
      if (!technology)
        throw new Error(
          `Technology ${id} is missing from the external asset map.`,
        );
      if (reference.inner.startsWith('<img class="proto-icon proto-icon-tech"'))
        return reference.full;
      if (!allowsTechnologyIcon(ancestorStackAtOffset(html, reference.index)))
        return reference.full;
      return reference.full.replace(
        reference.openingTag,
        `${reference.openingTag}${technologyIcon(technology)}`,
      );
    },
  );
}
...
const isReferenceTableSurface = (stack) =>
  stack.some(
    (entry) =>
      entry.tag === "table" &&
      entry.classes.some((className) => referenceTablePattern.test(className)),
  );
const isDesignedCalloutSurface = (stack) =>
  stack.some(
    (entry) =>
      entry.tag === "blockquote" ||
      entry.classes.some((className) => calloutClassPattern.test(className)),
  );
...
function allowsItemIcon(stack) {
  ...
  return (
    isCardOrMapSurface(stack) ||
    isReferenceTableSurface(stack) ||
    isDesignedCalloutSurface(stack)
  );
}
...
function addItemIcons(html) {
  return replaceElementsByClass(
    ...
    (reference) => {
      ...
      if (!item)
        throw new Error(`Item ${id} is missing from the external asset map.`);
      if (reference.inner.startsWith('<img class="proto-icon proto-icon-item"'))
        return reference.full;
      if (!allowsItemIcon(ancestorStackAtOffset(html, reference.index)))
        return reference.full;
      return reference.full.replace(
        reference.openingTag,
        `${reference.openingTag}${itemIcon(item)}`,
      );
    },
  );
}
...
function itemReference(itemId, label) {
  ...
  if (!item)
    throw new Error(
      `Required contextual item ${itemId} is missing from the effective asset map.`,
    );
...
function addRequiredContextIcons(html) {
  const titles = [
    {
      plain:
        '<span class="card-summary-title">Mall Logistics — buffer 900 Belts + 400 Sorters</span>',
    ...
    {
      plain:
        '<span class="card-summary-title">Mall Industry — buffer 50 Miners + 50 Smelters + 50 Mk.I Assemblers</span>',
  ...
  for (const title of titles) {
    ...
    if (!transformed.includes(title.plain))
      throw new Error(`${title.label} title could not be materialized.`);
...
function addExternalToolsLogo(html) {
  ...
  const marker =
    /<hr\/>\s*<h1 id="ref-tools">External Tools and Communities<\/h1>/;
  ...
  if (!marker.test(html))
    throw new Error("External Tools and Communities divider was not found.");
...
function validateMaterializationPolicy() {
  ...
  if (!item || !technology)
    throw new Error(
      "The icon policy fixture requires one item and one technology.",
    );
  ...
  for (const [name, [expectsItem, expectsTechnology]] of expected) {
    const region =
      findElementsByClass(materialized, `policy-test-${name}`)[0]?.inner || "";
  ...
  if (repeated !== materialized)
    throw new Error(
      "Icon placement policy materialization is not deterministic.",
    );
...
function bareRouteArrows(html) {
  ...
  for (const match of html.matchAll(tokenPattern)) {
    ...
    const inRoute = stack.some((entry) =>
      entry.classes.includes("route-chain"),
    );
    const inGlyph = stack.some((entry) =>
      entry.classes.includes("production-arrow-glyph"),
    );
...
function validate(html) {
  ...
  const techRefs = findElementsByClass(
    html,
    components.technologyReference.className,
  );
  for (const reference of techRefs) {
    ...
    if (technology) {
      ...
      const hasIcon = reference.inner.includes(
        'class="proto-icon proto-icon-tech"',
      );
      const stack = ancestorStackAtOffset(html, reference.index);
      const requiresIcon = allowsTechnologyIcon(stack);
      assert(
        !requiresIcon || hasIcon,
        `Technology ${id} is missing an icon on an approved surface.`,
      );
      assert(
        requiresIcon || isStory2DeferredSurface(stack) || !hasIcon,
        `Technology ${id} retains an icon on a Story 2 cleanup surface.`,
      );
      assert(
        !hasIcon || reference.inner.includes(expectedSource),
        `Technology ${id} has the wrong icon.`,
      );
      assert(
        stripTags(reference.inner).length > 0,
        `Technology ${id} has no visible label.`,
      );
    }
  }

  const protoRefs = findElementsByClass(
    html,
    components.itemReference.className,
  );
  for (const reference of protoRefs) {
    ...
    if (item) {
      ...
      const hasIcon = reference.inner.includes(
        'class="proto-icon proto-icon-item"',
      );
      const stack = ancestorStackAtOffset(html, reference.index);
      const requiresIcon = allowsItemIcon(stack);
      assert(
        !requiresIcon || hasIcon,
        `Item ${id} is missing an icon on an approved surface.`,
      );
      assert(
        requiresIcon || isStory2DeferredSurface(stack) || !hasIcon,
        `Item ${id} retains an icon on a Story 2 cleanup surface.`,
      );
      assert(
        !hasIcon || reference.inner.includes(expectedSource),
        `Item ${id} has the wrong icon.`,
      );
      assert(
        stripTags(reference.inner).length > 0,
        `Item ${id} has no visible label.`,
      );
    }
  }

  const arrows = findElementsByClass(
    html,
  ...
  for (const arrow of arrows) {
    ...
    if (producer) {
      assert(
        arrow.inner.includes(`src="${itemAsset(producer)}"`),
        `Producer ${id} has the wrong icon.`,
      );
      assert(
        elementTextByClass(arrow.inner, "visually-hidden") ===
          `Produced in ${producer.name}`,
        `Producer ${id} has the wrong accessible description.`,
      );
    }
  }
  ...
  assert(
    unwrappedArrows.length === 0,
    `Bare production arrows remain in routes: ${unwrappedArrows.join(" | ")}`,
  );
  ...
  assert(
    routeMaps.length > 0 &&
      routeMaps.every(
        (map) =>
          isNativeComponent(map, components.routeMap) &&
          !hasAttribute(map.openingTag, "role"),
      ),
    "A production map is not a native list.",
  );
  assert(
    routeRows.length > 0 &&
      routeRows.every(
        (row) =>
          isNativeComponent(row, components.routeRow) &&
          !hasAttribute(row.openingTag, "role"),
      ),
    "A production route is not a native list item.",
  );
  ...
  for (const [phase] of phaseBindings) {
    ...
    const expectedSource =
      binding.kind === "item" ? itemAsset(record) : technologyAsset(record);
    assert(
      Boolean(rail?.[1].includes('class="phase-icon phase-icon-rail"')),
      `Phase ${phase} rail icon is missing.`,
    );
    assert(
      Boolean(rail?.[1].includes(`src="${expectedSource}"`)),
    ...
    for (const link of phaseLinks)
      assert(
        link[1].includes(`src="${expectedSource}"`),
  ...
  const mallTitle = html.match(
    /<span class="card-summary-title">Mall Industry([\s\S]*?)<\/span><span class="card-summary-meta">/,
  );
  assert(Boolean(mallTitle), "Mall Industry card title is missing.");
  for (const itemId of [2301, 2302, 2303])
    assert(
      Boolean(mallTitle?.[1].includes(`data-item-id="${itemId}"`)),
      `Mall Industry title is missing item ${itemId}.`,
    );
  const logisticsTitle = html.match(
    /<span class="card-summary-title">Mall Logistics([\s\S]*?)<\/span><span class="card-summary-meta">/,
  );
  assert(Boolean(logisticsTitle), "Mall Logistics card title is missing.");
  for (const itemId of [2001, 2011])
    assert(
      Boolean(logisticsTitle?.[1].includes(`data-item-id="${itemId}"`)),
      `Mall Logistics title is missing item ${itemId}.`,
    );
  const gameLogos = findElementsByClass(html, "game-logo");
  assert(
    gameLogos.length === 2,
    "The guide must display the game logo at the title and External Tools sections.",
  );
  const steamLogoLinks = [
    ...html.matchAll(
      new RegExp(
        `<a\\b(?=[^>]*\\bhref="${escapeRegExp(steamStoreUrl)}")[^>]*>([\\s\\S]*?)<\\/a>`,
        "g",
      ),
    ),
  ];
  assert(
    steamLogoLinks.length === 2 &&
      steamLogoLinks.every(
        (link) => findElementsByClass(link[1], "game-logo").length === 1,
      ),
    "Both game logos must link to the Dyson Sphere Program Steam store page.",
  );
  ...
  for (const source of localImages) {
    assert(
      ...
      `Non-local image source found: ${source}`,
    );
    assert(
      fs.existsSync(path.join(root, source)),
      `Referenced image is missing: ${source}`,
    );
  }

  if (failures.length)
    throw new Error(
      `Prototype icon validation failed:\n- ${failures.join("\n- ")}`,
    );
  return {
...
if (mode === "write") {
  ...
  if (result.html !== source) {
    const difference = [...result.html].findIndex(
      (character, index) => character !== source[index],
    );
    throw new Error(
      `Prototype icon markup is incomplete or not idempotent near offset ${difference}. Run with --write.`,
    );
  }
  console.log(
    JSON.stringify(
      { status: "PASS", policySurfaces, ...validate(source) },
      null,
      2,
    ),
  );
